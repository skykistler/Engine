...
public class VertexRenderer {
	...
	public void start() {
		if (started) {
			System.out.println("Drawing already started but start() was called");
			return;
		}
		init();
		started = true;
	...
	public void finish(int mode) {
		if (!started) {
			System.out.println("Vertex renderer hasn't started drawing but finish() was called");
			return;
		}
		if (vertexCount == 0) {
			started = false;
	...
	public void addVertex(double x, double y, double z) {
		...
		if (bufferCount >= bufferLimit) {
			finish();
			System.out.println("Reached Modeler buffer limit");
			start();
		}
	}
